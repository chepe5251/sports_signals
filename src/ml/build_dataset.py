...
def add_team_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    long["drawrate_5"]= g["draw"].apply(lambda s: s.shift(1).rolling(N_FORM).mean())

    home_feats = long[long["is_home"]==1][["match_id","avg_gf_5","avg_ga_5","pts_5","winrate_5","drawrate_5"]].copy()
    home_feats.columns = ["match_id","home_avg_gf_5","home_avg_ga_5","home_pts_5","home_winrate_5","home_drawrate_5"]

    away_feats = long[long["is_home"]==0][["match_id","avg_gf_5","avg_ga_5","pts_5","winrate_5","drawrate_5"]].copy()
    away_feats.columns = ["match_id","away_avg_gf_5","away_avg_ga_5","away_pts_5","away_winrate_5","away_drawrate_5"]
    ...
    out["diff_drawrate_5"] = out["home_drawrate_5"] - out["away_drawrate_5"]
